...
public class SEED256Util {
    ...
    public static String doEncrypt(String content) throws Exception{
        return new SEED256Util().encrypt(content);
    }

    public static String doEncrypt(String content, String key) throws Exception{
        return new SEED256Util(key).encrypt(content);
    }
    ...
    public static String doDecrypt(String content) throws Exception{
        return new SEED256Util().decrypt(content);
    }

    public static String doDecrypt(String content, String key) throws Exception{
        return new SEED256Util(key).decrypt(content);
    }
     ...
     * Implement
     *************************/
    public SEED256Util() throws Exception {
        try{
    ...
    public static byte[] SeedEncrypt (byte[] password, int[] seedRoundKey) throws Exception {

        byte pbCipher[] = new byte[TEXT_SIZE];


        SeedEncrypt (password, seedRoundKey, pbCipher);

        return pbCipher;
    }

    public static byte[] SeedEncrypt (String password, int[] seedRoundKey) throws Exception {
    ...
    public static String runDecrypt(byte[] hash, int[] seedRoundKey) throws Exception {
        ...
        if (hash.length <= TEXT_SIZE) {
            SeedDecrypt (hash, seedRoundKey, result);
        } else {
            ...
            for (int i = 0; i < loop; i++) {
                ...
                SeedDecrypt (temp2, seedRoundKey, temp);
        ...
        if(loop == 0) loop = 1;
        result = removePadding (result, TEXT_SIZE * loop);
        ...
        try{
            ...
            if(password.length () == 1) {
                ...
                if (pwdByte.length <= TEXT_SIZE) {
                    ...
                    for (int i = 0; i < loop; i++) {
                        if ((pwdByte.length - end) <= TEXT_SIZE) {
                            ...
                            temp = SeedEncrypt (addPadding (new String(pwdByte, i * TEXT_SIZE, term, charset).getBytes (charset), TEXT_SIZE), seedRoundKey);
                            for (int j = 0; j < temp.length; j++)
                                result[(i * TEXT_SIZE) + j] = temp[j];
                        } else {
                            temp = SeedEncrypt (new String(pwdByte, i * TEXT_SIZE, term, charset), seedRoundKey);
                            for (int j = 0; j < temp.length; j++)
                                result[(i * TEXT_SIZE) + j] = temp[j];
                        }
                        end += TEXT_SIZE;
                ...
                for (byte hash : result) {
                    resultHash += addZero (Integer.toHexString (0xff & hash), 2);
                }
                return resultHash;
            }
     ...
     *
     **************************************************/
    private String ansiName = "ANSI-X.923-Padding";
