...
class Help extends TaskHelper{
    ...
    boolean isCommandable(String applicationName){
        return [CommanderHelper.APPLICATION_INSTALLER_MAKER, CommanderHelper.APPLICATION_HOYA].contains(applicationName)
    }
    ...
    Map<String, String> collectValueNames(Class clazz, String propertyPrefixFirstName, Map<String, String> resultPropertyMap){
        Dependency reflectInfo = config.reflectionMap[clazz]
        if (reflectInfo){
